...
def analyse_ndvi_simple(annee1, annee2):
    ...
    vectoriser_changements(changements, transform, crs, f'changements_NDVI_{annee1}_{annee2}')
...
def analyse_avancee(annee1, annee2, n_clusters=4):
    ...
    - Clustering K-means
    Comparaison simple des méthodes.
    """
    ...
    for indice in indices:
        ...
        ind2, _, _, _ = charger_indice(indice, annee2)
        delta = ind2 - ind1
        delta[np.isnan(delta)] = 0  # gérer NoData
        data[indice] = delta
    ...
    seg_map[data['NDVI'] > threshold_seg] = 2  # amélioration
    seg_map[data['NDVI'] < -threshold_seg] = 1  # dégradation
    ...
    features_clean_k = features_clean.copy()
    kmeans = KMeans(n_clusters=n_clusters, random_state=42, n_init=10)
    clusters = np.full(h * w, np.nan)
    clusters[mask_valid] = kmeans.fit_predict(features_clean_k)
...
if __name__ == "__main__":
    ...
    for i in range(len(ANNEES)-1):
        ...
        # Analyse avancée
        analyse_avancee(annee1, annee2, n_clusters=4)
